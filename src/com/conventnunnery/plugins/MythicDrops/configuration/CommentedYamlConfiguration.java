...
public class CommentedYamlConfiguration extends YamlConfiguration {
	...
	public void setDefaults(Configuration defaults) {
		...
		ConfigurationSection section = defaults
				.getConfigurationSection("comments");

		if (section != null) {
			for (String comment : section.getKeys(false)) {
				String path = comment.replace("[ \t]*~[ \t]*", ".");
				String[] lines = section.getString(comment).split(
						"[ \t]*\\n[ \t]*");
				addComment(path, lines);
			}
		}

		defaults.set("comments", null);
